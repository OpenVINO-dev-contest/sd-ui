...
if model_path.exists():
    ov_pipe = OVStableDiffusionPipeline.from_pretrained(model_path, device=current_device, compile=False)
    ov_pipe.reshape(batch_size=1, height=512, width=512, num_images_per_prompt=1)
else:
    ov_pipe = OVStableDiffusionPipeline.from_pretrained(model_id, device=current_device, export=True, compile=False)
    ov_pipe.reshape(batch_size=1, height=512, width=512, num_images_per_prompt=1)
    ov_pipe.save_pretrained(model_path)

def generate_from_text(text, negative_text, num_steps, scale, _=gr.Progress(track_tqdm=True)):
    start = time.perf_counter()
    output = ov_pipe(prompt=text, negative_prompt=negative_text, num_inference_steps=num_steps, guidance_scale=scale).images[0]
...
with gr.Blocks() as demo:
    gr.Markdown(
        "# Image Generation with Stable Diffusion using OpenVINO.\n"
    )

    with gr.Row():
        with gr.Column(scale=4):
            text = gr.Textbox(
                value="red car in snowy forest",
            ...
            model_output = gr.Image(label="Result", type="pil")
        ...
        with gr.Column(scale=1):
            ...
            num_steps = gr.Slider(
                minimum=1, maximum=100, value=20, step=1, interactive=True, label="Number of steps",
            )
            guidance_scale = gr.Slider(
                minimum=0.5, maximum=30.0, value=7.5, step=0.5, interactive=True, label="Guidance scale",
            )

    button_submit.click(generate_from_text, [text, negative_text, num_steps, guidance_scale], [model_output, performance])
    button_clear.click(reset, [model_output, performance], [model_output, performance])
    device.change(select_device, [device, text], [text])

if __name__ == "__main__":
    try:
        demo.launch(server_name = '10.3.233.99', enable_queue=True, share=False, height=800)
    except Exception:
